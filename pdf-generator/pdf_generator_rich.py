...
Usage:
    CLI: python3 pdf_generator_rich.py --cli
    GUI: python3 pdf_generator_rich.py --gui (or just run without arguments)
"""
...
def preprocess_rich_text(text):
    """
    Normalize input text to ReportLab-compatible rich text.

    Supports Markdown and rich HTML conversion.
    """
    if not text:
        return ""
    ...
    if has_markdown and not has_html:
        processed_text = convert_markdown_to_html(processed_text)

    # Convert rich HTML to ReportLab-compatible format if needed
    return convert_rich_html_to_reportlab(processed_text)


def create_rich_paragraphs(text, style):
    """Create ReportLab Paragraph objects from multiline rich text."""
    processed_text = preprocess_rich_text(text)
    if not processed_text.strip():
        return []

    paragraphs = []
    for para_text in processed_text.split('\n\n'):
        cleaned = para_text.strip()
        if cleaned:
            cleaned = cleaned.replace('\n', '<br/>')
            paragraphs.append(Paragraph(cleaned, style))
    return paragraphs
...
def generate_pdf(config):
    ...
    footer_style = ParagraphStyle(
        ...
        alignment=get_alignment(config.footer_alignment),
        fontName='DejaVuSans',
        leading=config.footer_font_size * 1.3,
        spaceAfter=2
    )
    ...
    # Add body text (rich-text and multiline support)
    story.extend(create_rich_paragraphs(config.body_text, body_style))
...
def run_cli():
    ...
    parser.add_argument('--title', default='Document Title',
                       help='Document title text (supports rich-text and \\n for multiline)')
    ...
    # Footer
    parser.add_argument('--footer', help='Footer text (supports rich-text and \\n for multiline)')
    ...
    config.title = args.title.replace('\\n', '\n')
    ...
    if args.footer:
        config.footer_text = args.footer.replace('\\n', '\n')
...
def run_gui():
    ...
    class PDFGeneratorGUI:
        def __init__(self, root):
            self.root = root
            self.root.title("PDF Generator (Rich Text)")
        ...
        def create_preview_tab(self, parent):
            ...
            # Canvas for preview
            canvas_frame = ttk.Frame(parent, relief='sunken', borderwidth=2)
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == '--cli':
        ...
        print(__doc__)
        print("\nFor CLI help: python3 pdf_generator_rich.py --cli --help")
        sys.exit(0)
    else:
        # Default to GUI
        run_gui()
